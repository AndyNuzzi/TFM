...
class Modern20142015PdfParser(BasePdfParser):
    ...
    def parse(self, content: ExtractedPdfContent) -> ParsedDataset:
        pages = [self._normalize_text(page) for page in content.pages_text if page]
        full_text = "\n".join(pages)
    ...
    def _parse_subject_results(self, dataset: ParsedDataset, page_text: str) -> None:
        ...
        for line in lines:
            ...
            if upper.startswith("PROFESORADO QUE IMPARTE EN EL PLAN DE ESTUDIO"):
                break

            if upper.startswith("OTROS INDICADORES DEL PROFESORADO"):
                break
            ...
            if row_pattern.match(candidate):
